...
class FlatFileDB(qaapDB):
    ...
    def post_exists(self, post_title: str) -> bool:
        ...
        """
        path = Path(self.root,"documents",post_title)
    ...
    def comment_exists(self, post_title: str, note_title: str) -> bool:
        ...
        """
        path = Path(self.root,"documents",post_title,note_title)
        return path.exists()
    ...
    def get_document(self, post_title: str) -> tuple[str,str]:
        ...
        try:
            with open(Path(self.root,"documents",post_title,"document.txt"),encoding="utf-8") as file:
                post_content = file.read()

            if Path(self.root,"documents",post_title,"icon.png").exists():
                with open(Path(self.root,"documents",post_title,"icon.png"), "rb") as file:
                    content = file.read()
                    icon = str(base64.urlsafe_b64encode(content).decode("utf-8"))
    ...
    def get_document_medias(self, post_title: str, includes: list[str] = None) -> list[str]:
        ...
        paths = sorted(Path(self.root,"documents",post_title).glob("*.png"))
    ...
    def get_notes_for_post(self, post_title: str, perpage: int = 0, page: int = 0) -> list[tuple[str,str,str]]:
        ...
        notes = sorted(Path(self.root,"documents",post_title,"").rglob("note.txt"))
    ...
    def get_note_medias(self, post_title: str, note_title: str, includes: list[str] = None) -> list[str]:
        ...
        paths = sorted(Path(self.root, "documents", post_title, note_title).glob("*.png"))
    ...
    def write_post(self, post_title: str, content: str, icon: str = "", medias: list[(str,str)] = []) -> bool:
        ...
        folder_path = Path(self.root,"documents",post_title)
        ...
        try:
            ...
            if icon:
                with safer.open(Path(folder_path,"icon.png"), "wb") as file:
                    file.write(base64.b64decode(icon))
    ...
    def write_comment(self, post_title: str, note_title: str, content: str, medias: list[(str,str)] = []) -> bool:
        """
            ...
            Args:
                post_title (str): Name of the document to write the note for.
                note_title (str): Name of the note_title.
        ...
        folder_path = Path(self.root,"documents",post_title,note_title)
    ...
    def get_all_documents_data(self) -> list[dict[str,str]]:
        ...
        posts_paths = sorted(Path(self.root,"documents").rglob("document.txt"))
